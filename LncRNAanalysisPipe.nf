...
if (params.help) {
    ...
    log.info print_yellow('Copyright (c) 2013-2020, Sun Yat-sen University Cancer Center.')
    log.info '------------------------------------------------------------------------'
    exit 0
}
...
//Prepare annotations
    ...
    process combine_public_annotation {
         publishDir path: { params.out_folder + "/Result/Combined_annotations" }, mode: 'copy', overwrite: true
        input:
...
//    file "lncRNA.final.v2.map" into finalLncRNA_map
    ...
    if(params.species=="human"){
        ...
        perl !{baseDir}/bin/rename_proteincoding.pl !{gencode_protein_coding_gtf} | grep -w exon > protein_coding.final.gtf
